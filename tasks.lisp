...
(defun start-server (&rest args &key name (show-remaining-task-limit 100) (log-to-file t) threaded to-stage endpoint)
  (let* ((plan (load-plan :name name))
         (task-graph-callback (lambda (stream)
                                (build-task-graph plan stream)))
         (connection-path (make-connection-pathname nil))
	(args (alexandria:remove-from-plist args :name)))
...
(defun start-client (&rest args &key name (log-to-file t) to-stage threaded dont-execute)
  (let* ((plan (load-plan :name name))
         (task-graph-callback (lambda (stream)
                                (build-task-graph plan stream)))
         (connection-path (make-connection-pathname nil))
...
(defun run (&rest args &key name test)
  (let* ((*test* test)
         (plan (load-plan :name name))
